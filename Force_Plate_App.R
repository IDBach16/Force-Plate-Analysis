...
ui <- fluidPage(
  ...
  navbarPage(
    ...
    tabPanel("Predictive Modeling",
             sidebarPanel(
               h3("Predictive Model Overview"),
               p("This section shows a linear regression model built to predict Jump Height based on Concentric Mean Force and RSI."),
               p("The data is split into training and testing sets, with 70% of the data used for training the model and 30% for testing."),
               p("The performance of the model is evaluated using Root Mean Squared Error (RMSE), and the predictions are visualized against actual values."),
               tableOutput("modelMetricsTable")
             ),
             mainPanel(
               h3("Model Performance"),
               plotlyOutput("predVsActualPlot")
             ))
  )
)
...
server <- function(input, output, session) {
  ...
  set.seed(123)
  train_indices <- sample(1:nrow(jump_data), 0.7 * nrow(jump_data))
  train_data <- jump_data[train_indices, ]
  test_data <- jump_data[-train_indices, ]
  
  # Build a linear regression model to predict Jump Height
  model <- lm(Jump.Height..Imp.Mom..in.Inches ~ Concentric.Mean.Force..N. + RSI.Modified..m.s., data = train_data)
  
  # Evaluate the model
  predictions <- predict(model, test_data)
  actuals <- test_data$Jump.Height..Imp.Mom..in.Inches
  
  # Calculate RMSE
  rmse <- sqrt(mean((predictions - actuals)^2))
  
  # Output RMSE as a table
  ...
  # Visualize the predictions vs actuals using plotly
  output$predVsActualPlot <- renderPlotly({
    p <- ggplot(test_data, aes(x = actuals, y = predictions)) +
      geom_point(aes(text = paste0("PlayerID: ",PlayerId,"<br>",
                                   "Field Position: ",Field.Position,"</br>",
                                   "Weight:  ",Weight..kg.,"</br>"
      )),size = 3, alpha = 0.7) +
      geom_abline(slope = 1, intercept = 0, color = "red") +
      labs(title = "Predictions vs Actuals for Jump Height", x = "Actual Jump Height", y = "Predicted Jump Height") +
      theme_minimal()
